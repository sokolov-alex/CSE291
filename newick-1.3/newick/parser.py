...
class AbstractHandler(object):
    ...
    def new_edge(self, bootstrap, length):
        ...
        and total_len is given if these are parsed, otherwise these are
        None.'''
        pass
...
class _Parser(object):
    ...
    def parse_edge(self):
        ''' Parse a single edge, either leaf [bootstrap] [: branch-total_len]
        or tree [bootstrap] [: branch-total_len]. '''
        if self.lexer.peek_token(tokens.LParen):
            self.parse_node()
        else:
            self.parse_leaf()
